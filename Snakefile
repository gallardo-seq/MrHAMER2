...
## Optional parameters ##
#########################
chimera_buster_status = config.get("chimera_buster_on", True)
CB_param = "-m "+str(config.get("chimera_buster_MT"))+" -c "+str(config.get("chimera_buster_HS"))
minimap2_param = config.get("minimap2_param")
...
rule clusters:
    input:
        expand("{name}/targets.bed", name=sample_name),
        expand("{name}/stats/{target}_vsearch_cluster_stats.tsv", name=sample_name, target=target),
	expand("{name}/stats/{target}_output.csv", name=sample_name, target=target)
...
rule count_cluster:
    input:
        "{name}/stats/{target}_vsearch_cluster_stats.tsv"
    output:
        "{name}/stats/{target}_output.csv"
    shell:
        "mlr --tsv filter -S '$n>=5' then cut -f id_cluster,n {input} > {output}"
...
rule chimera_buster:
    input:
        pre = expand("{name}/clustering/{target}/clusters_consensus.fasta", name=sample_name, target=target),
	final = expand("{name}/clustering_consensus/{target}/clusters_consensus.fasta", name=sample_name, target=target)
    output:
        NC_csv = "CB_{target}_non_chimeras.csv",
        C_csv = "CB_{target}_chimeras.csv",
        NC_txt = "CB_{target}_nonchimera_list.txt",
        C_txt = "CB_{target}_chimera_list.txt"
    params:
        CB_param= CB_param 
    shell:
        """
	Chimera_Buster {params.CB_param} {input.final} {input.pre} CB_{target}
	"""
...
rule mv_CB_file:
	...
	C_txt = "CB_{target}_chimera_list.txt"
    output:
        NC_csv = "{name}/Chimera_Buster/CB_{target}_non_chimeras.csv",
        C_csv = "{name}/Chimera_Buster/CB_{target}_chimeras.csv",
        NC_txt = "{name}/Chimera_Buster/CB_{target}_nonchimera_list.txt",
        C_txt = "{name}/Chimera_Buster/CB_{target}_chimera_list.txt"
    shell:
        """
        mv {input.NC_csv} {output.NC_csv} && mv {input.C_csv} {output.C_csv} && mv {input.NC_txt} {output.NC_txt} && mv {input.C_txt} {output.C_txt}
	"""
